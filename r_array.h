...
typedef char * (*ra_value_printer)(void * value);
/**
...
char * vector_printer(void * value) {
    ...
    static char output_buffer[256]; // best practice to use a static buffer
...
void ra_printf_f(r_array * ra, ra_value_printer printer) {
    ...
    char * value = printer(ra_get(ra, 0));
    printf("%s", value);
    ...
    for(size_t i = 1; i < ra->count; i++) {
        char * value = printer(ra_get(ra, i));
        printf(", %s", value);
    }
